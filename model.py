...
class AgePredictionModel(nn.Module):
    ...
    def __init__(
        ...
        self.backbone = timm.create_model(
            ...
            num_classes=0,  # no classifier head, we'll add our own
            global_pool="avg",
        )
    ...
    def forward(
        ...
        age_pred = self.age_regressor(feats).squeeze(-1)  # (B,)

        if self.use_soft_labels and self.age_distribution_head is not None:
            logits = self.age_distribution_head(feats)  # (B, max_age+1)
        else:
            logits = None
